...
def create_fresh_lb_import_dict():
...
def generate_device_json(device_ws, grey_net):
    ...
    # This is the basic data structure which defines all the fields populated when importing a device into NetBox
    device_import_format = {
        "name":"",
        "manufacturer":"",
        "device_type":"",
    ...
    # Set name of site based on location of switch
...
def generate_lb_json(device_ws, grey_net):
    
    # This is the basic data structure which defines all the fields populated when importing a loopback address into NetBox
    lb_import_format = {
        "address":"",
        "status":"",
        "role":"",
    ...
    for row in device_ws.iter_rows(min_row=2, max_row=device_ws.max_row, values_only=True): # min_row 0 & 1 both include headers
        ...
        lb_import_format = create_fresh_lb_import_dict()

        if (row[8] == None): 
            lb_import_format["address"] = row[9] + "/24" # Grey switches in MR + PQ use Loopback2 addresses?
        else:
            lb_import_format["address"] = row[8] + "/24"
        lb_import_format["status"] = "active"
        lb_import_format["role"] = "loopback"
        lb_import_format["device"] = row[0]
        lb_import_format["description"] = row[0] + " loopback address"

        lb_json_list.append(dict(lb_import_format))
...
def generate_int_json(interface_ws, device_ws, grey_net):
    ...
    # This is the basic data structure which defines all the fields populated when importing an interface into NetBox
    ...
    for row in interface_ws.iter_rows(min_row=2, max_row=interface_ws.max_row, values_only=True): # min_row 0 & 1 both include headers
        ...
        if (row[2] == "enabled"):    
            ...
            if row[10] == "enabled": 
                int_import_format["cf_pim_enabled"] = True
...
def main():
    ...
    # Generate Device Loopback Address list
    lb_json_list = generate_lb_json(device_ws, grey_net)
    ...
    # Generate device interface list
    ip_json_list = generate_ip_json(interface_ws, device_ws, grey_net) # device worksheet required to get interface type from switch model

    # Write data to JSON files
    path = "C:\\Users\\caddem01\\OneDrive - BBC\\BBC\\Code\\Python Code\\netbox-cmn-import-script\\uploads\\"

    # Set prefix of save file names:
    network = "_red_blue"
    if grey_net == True: network = "_grey"

    # Save the device import JSON
    save_file_name = site + network + "_device_import.json"
    with open(path + save_file_name, "w") as open_file:
        json.dump(device_json_list, open_file, indent=2) # pretty JSON file

    # Save the loopback address import JSON
    save_file_name = site + network + "_lb_import.json"
    with open(path + save_file_name, "w") as open_file:
        json.dump(lb_json_list, open_file, indent=2) # pretty JSON file

    # Save the device interface import JSON
    save_file_name = site + network + "_int_import.json"
    with open(path + save_file_name, "w") as open_file:
        json.dump(int_json_list, open_file, indent=2) # pretty JSON file

    # Save the interface aIP address import JSON
    save_file_name = site + network + "_ip_import.json"
    with open(path + save_file_name, "w") as open_file:
